...
class KnowledgeRetriever {
  ...
  constructor(dbPath?: string) {
    // Use the path from environment variable or construct the absolute path
    let defaultPath: string;

    // The env variable uses relative path, so resolve it
    if (process.env.KNOWLEDGE_DB_PATH) {
      // Resolve relative path from env variable
      defaultPath = path.resolve(process.env.KNOWLEDGE_DB_PATH);
    } else {
      // Fallback to constructing path from project root
      defaultPath = path.join(process.cwd(), '..', 'data', 'kb_index.sqlite');
    }

    const dbFile = dbPath || defaultPath;
    console.log('Attempting to open database at:', dbFile);
  ...
  private queryRelevantChunks(filter: KnowledgeFilter): KnowledgeChunk[] {
    let query = `
      ...
      WHERE subject_key = ?
    `;
    const params: any[] = [filter.subjectKey];

    // Add notion filter only if provided and not empty
    if (filter.notionKey && filter.notionKey.trim() !== '') {
      query += ` AND notion_key = ?`;
      params.push(filter.notionKey);
    }
